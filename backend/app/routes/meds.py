...
async def delete_treatment_by_name(
    ...
    try:
        ...
        result = await db.treatments.delete_one(
            {"name": name, "user_id": mock_user["_id"]}
        )

        if result.deleted_count == 0:
            raise HTTPException(
                status_code=404, detail="No se ha encontrado el tratamiento"
            )
...
async def delete_sporadic_medication_by_name(
    ...
    try:
        ...
        result = await db.sporadic_medication.delete_one(
            {"name": name, "user_id": mock_user["_id"]}
        )

        if result.deleted_count == 0:
            raise HTTPException(
                status_code=404, detail="No se ha encontrado la medicación esporádica"
            )
